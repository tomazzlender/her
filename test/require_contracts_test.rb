...
# Her.require_contracts: every component must declare its contract (block
# attrs or frontmatter); contract-less templates compile with an EMPTY
# contract, so each @x reference is a precise load error.
class RequireContractsTest < Minitest::Test
  def with_required_contracts
    Her.require_contracts = true
    yield
  ensure
    Her.require_contracts = false
  end

  def test_contract_less_template_referencing_assigns_fails_at_load
    with_required_contracts do
      ...
      assert_match(/contracts are required \(Her\.require_contracts\)/, error.message)
  ...
  def test_block_attrs_satisfy_the_requirement
    with_required_contracts do
      mod = component_module do
        component :ok do
  ...
  def test_frontmatter_satisfies_the_requirement
    with_required_contracts do
      Dir.mktmpdir do |dir|
  ...
  def test_per_definition_opt_out
    with_required_contracts do
      mod = component_module do
        component :legacy, require_contract: false do
          template "<p>{@anything}</p>"
        end
      end
      assert_equal "<p>1</p>", render(mod.legacy(anything: 1))
    end
  end

  def test_per_definition_opt_in_without_global_flag
    error = assert_raises(Her::CompileError) do
      component_module do
        component :strict_one, require_contract: true do
          template "<p>{@x}</p>"
        end
      end
    end
    assert_match(/contracts are required/, error.message)
  end
