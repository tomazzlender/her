...
plugin in [`intellij/her/`](intellij/her) and you're done — it bundles
the TextMate grammar (highlighting works on install) and starts
`bundle exec her lsp` per project, reading the boot file from a
`.her-lsp` file at the project root (or `config/boot.rb` when present).
See its README for build steps and troubleshooting (file-type
associations that shadow the grammar, version-manager PATH issues).

**Community Edition** (no LSP API, so the plugin can't load): wire the
two halves manually —

- highlighting: `Settings → Editor → TextMate Bundles → +` and select
  this repo's `editors/vscode/her` directory (JetBrains IDEs import
  VS Code extension folders as bundles). Holes highlight as Ruby only if
  a Ruby TextMate bundle is also loaded; the HTML/component/slot
  structure highlights regardless.
- language server: install the
